...
def run_model(J, Q, NFILT, KERNEL, HIDDEN, NGRUS, BATCH, LR, L2):
    # DATA INITIALIZATION
    dataset_train = Dataset('./train/', 'train_small_1_fnusa.csv')
    dataset_test = Dataset('./test/', 'valid_fnusa.csv')
    ...
    exmpl = next(iter(dataset_train))
    # print(exmpl[0].shape)  # 16384
    exmpl = np.expand_dims(exmpl[0], axis=0)
    exmpl = torch.Tensor(exmpl)
    exmpl = exmpl.to(DEVICE)
    # print(exmpl.shape)  # torch.Size([1, 16384])
    exmpl = first_order_scattering(exmpl, DEVICE, J, Q)
    # print(exmpl.shape)  # torch.Size([1, 1, x, y])
    kern = exmpl.shape[-2]
    # print(kern)  # x
    ...
    scattering = Scattering1D(J, 2**14, Q)
    ...
    for epoch in range(EPOCHS):
        ...
        for i, (x, t) in enumerate(tqdm(TRAIN)):
            ...
            x = x.to(DEVICE)
            Sx = scattering(x)
            x = Sx[:, np.array(order1), :]
            x = x[:, :, :, :]
            t = t.to(DEVICE)
            y = model(x)[:, -1, :]
            J_loss = loss(input=y, target=t)
            J_loss.backward()

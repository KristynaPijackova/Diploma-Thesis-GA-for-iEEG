...
class Generations:
    def __init__(self, population_size, timeout=3000):
    ...
    def mutate(individual, mutation_rate=0.01):
        ...
        if random.random() < mutation_rate:
            individual.parameters['NPERSEG'] = random.randint(5, 9)
        if random.random() < mutation_rate:
        ...
        if random.random() < mutation_rate:
            individual.parameters['KERNEL'] = random.choice([3, 5, 7, 11, 15])
        if random.random() < mutation_rate:

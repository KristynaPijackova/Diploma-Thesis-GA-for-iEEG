...
class Statistics(object):
    ...
    def evaluate(self):
        ...
        print('AUROC: {}\tAUROC mean: {}'.format(AUROC, np.mean(AUROC)))
        print('AUPRC: {}\tAUPRC mean: {}'.format(AUPRC, np.mean(AUPRC)))
        print('AUPRC chance: {}'.format(AUPRC_chance))
        print("-" * 60)
        self.reset()
        return KPS, F1, CONF, np.mean(AUROC), np.mean(AUPRC), AUROC, AUPRC